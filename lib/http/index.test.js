...
describe('http.js', () => {
  ...
  describe('request()', () => {
    ...
    it('should throw an error if response is not json based', async () => {
      ...
      expect(error).toMatchObject({
        code: 0,
        message: new SyntaxError('Unexpected token < in JSON at position 0'),
      });
    });
  ...
  describe('requestWithRetry()', () => {
    it('should allow to retry a request if the res code is 403', async () => {
      nock(globalThis.__API_URL__)
        .get('/test')
        .query({
          clientId: globalThis.__CLIENT_ID__,
      ...
      nock(globalThis.__API_URL__)
        ...
        .reply(200, { accessToken: 'foo', refreshToken: 'bar' });
      ...
      const res = await client.requestWithRetry({
        method: 'GET',
        resource: '/test',
        auth: {
          refreshToken: 'bar',
    ...
    it('should throw an error when retrying request without a refresh ' +
      'token', async () => {
